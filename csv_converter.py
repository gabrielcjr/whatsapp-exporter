...
# Filter out lines where the last field is empty or only contains the end pattern
filtered_messages = [message for message in messages if message[2].strip() and not message[2].strip() == current_message[1]]

# Write the extracted messages to a CSV file
with open(output_csv_path, 'w', newline='', encoding='latin-1') as output_csv:
    csv_writer = csv.writer(output_csv)
    csv_writer.writerows(filtered_messages)
